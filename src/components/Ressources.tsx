import RsCard from "./RsCard";

export default function Ressources() {
	return (
		<section className="px-4 mt-32">
			<h2>Ressources</h2>
			<p className="mt-5">Voici les sites ou vidéos qui m’ont aidé et m’aident à me former. Les explications y sont claires et permettent de progresser plus facilement sur certains concepts ardus.</p>
			<div className="flex flex-col md:flex-row justify-center items-center gap-5 mt-5">
				<RsCard
					RsName="Pierre Giraud"
					RsIcon="/icons/pierregiraud.png"
					RsPic="/photos/pierregiraud.png"
					RsLink="https://www.pierre-giraud.com/"
					RsLinkYT="https://www.youtube.com/pierregiraudcom"
					RsNote="Son cours m'a fait aimé JavaScript"
				/>
				<RsCard
					RsName="École du web"
					RsIcon="/icons/ecoleduweb.png"
					RsPic="/photos/pierregiraud.png"
					RsLink="https://www.ecole-du-web.net/"
					RsLinkYT="https://www.youtube.com/@EcoleduWeb"
					RsNote="J'ai particulièrment aimé les vidéos sur le CSS"
				/>
				<RsCard
					RsName="Code Concept"
					RsIcon="/icons/codeconcept.jpg"
					RsPic="/photos/pierregiraud.png"
					RsLink="https://codeconcept.teachable.com/"
					RsLinkYT="https://www.youtube.com/@codeconcept"
					RsNote="Pour moi la meilleure vidéo sur React qui existe"
				/>
				<RsCard
					RsName="DrcMind"
					RsIcon="/icons/drcmind.svg"
					RsPic="/photos/pierregiraud.png"
					RsLink="https://drcmind.com/"
					RsLinkYT="https://www.youtube.com/@DrcMind"
					RsNote="Son cours m'a fait aimé JavaScript"
				/>
			</div>
		</section>
	);
}
